...
combined.object.final <- readRDS("./combined_object_no_doublets")
...
# Extract expression data for GENE_A
gene_A_data <- combined.object.final@assays$RNA[gene_A, ]

# Extract module score
module_score <- combined.object.final$GO_TCR_signalling1

# Calculate correlation
correlation <- cor(gene_A_data, module_score)
correlation

# Create a scatterplot
plot1 <- plot(gene_A_data, module_score, 
              main = paste("Correlation between", gene_A, "and Module Score for TCR signalling"),
              xlab = gene_A, ylab = "Module Score TCR signalling function [GO:0050862]")
plot1
ggsave("./figures/Scatterplot_SGK1_and_module_score_TCRsignalling.pdf",plot1, units = "cm", width = 15, height = 15)


# Perform a correlation test
cor_test_result <- cor.test(gene_A_data, module_score)
...
network_data <- rbind(network_data, data.frame(Gene = "SGK1", ModuleScore = "SGK1", Correlation = 1))
network_data

# Add rows for each TCR gene
for (gene in TCR_sig_GO_genes) {
  # Extract expression data for the current TCR gene
  gene_data <- combined.object.final@assays$RNA[gene, ]
  
  # Calculate correlation with SGK1
  correlation <- cor(gene_data, combined.object.final@assays$RNA["SGK1", ])
  
  # Round the correlation value to 3 decimal places
  correlation_rounded <- round(correlation, 3)
  
  # Add a row to the data frame
  network_data <- rbind(network_data, data.frame(Gene = gene, ModuleScore = "SGK1", Correlation = correlation_rounded))
  
  # Print the correlation value
  cat(sprintf("Correlation between %s and SGK1: %.3f\n", gene, correlation_rounded))
}

# Remove the first row from network_data
network_data <- network_data[-1, ]

# Create an igraph graph object
graph <- graph_from_data_frame(d = network_data, directed = FALSE)

# Plot the network
plot(graph, main = "Gene Module Network for SGK1 and genes from TCR signalling function [GO:0050862]",
     vertex.label = V(graph)$Gene,
     vertex.color = ifelse(V(graph)$Gene == "SGK1", "red", "blue"),
     vertex.size = 28,
     edge.label = E(graph)$Correlation,
     edge.color = ifelse(E(graph)$Correlation > 0, "green", "red"))
...
plot1 <- plot(gene_A_data, module_score, 
              ...
              xlab = gene_A, ylab = "Module Score TCR signalling function [GO:0050862]")
plot1
ggsave("./figures/Scatterplot_NFAT5_and_module_score_TCRsignalling.pdf",plot1, units = "cm", width = 15, height = 15)
...
# Add a row to the data frame
network_data <- rbind(network_data, data.frame(Gene = "NFAT5", ModuleScore = "NFAT5", Correlation = 1))
network_data
...
for (gene in TCR_sig_GO_genes) {
  ...
  # Calculate correlation with SGK1
  correlation <- cor(gene_data, combined.object.final@assays$RNA["NFAT5", ])
  
  # Round the correlation value to 3 decimal places
  correlation_rounded <- round(correlation, 3)
  
  # Add a row to the data frame
  network_data <- rbind(network_data, data.frame(Gene = gene, ModuleScore = "NFAT5", Correlation = correlation_rounded))
  
  # Print the correlation value
  cat(sprintf("Correlation between %s and NFAT5: %.3f\n", gene, correlation_rounded))
}
...
# Plot the network
plot(graph, main = "Gene Module Network for NFAT5 and genes from TCR signalling function [GO:0050862] \n Statistics: t = 5.4477, p-value = 5.246e-08, corr = 0.05923977",
     vertex.label = V(graph)$Gene,
     vertex.color = ifelse(V(graph)$Gene == "NFAT5", "red", "blue"),
     vertex.size = 28,
     edge.label = E(graph)$Correlation,
     edge.color = ifelse(E(graph)$Correlation > 0, "green", "red"))
...
# Extract module score
module_score <- combined.object.final$Exhaustion1
...
# Add rows for each gene from module
for (gene in Exhaustion_GO_genes) {
...
# Plot the network
plot(graph, main = "Gene Module Network for NFAT5 and genes from Exhaustion Kusnadi, Anthony et al., Sci. immunology \n Statistics: t = 8.0742, p-value = 7.558e-16, corr = 0.07983669",
     vertex.label = V(graph)$Gene,
     vertex.color = ifelse(V(graph)$Gene == "NFAT5", "red", "blue"),
     vertex.size = 28,
     edge.label = E(graph)$Correlation,
...
# Plot the network
plot(graph, main = "Gene Module Network for SGK1 and genes from Exhaustion Kusnadi, Anthony et al., Sci. immunology \n Statistics: t = 8.0742, p-value = 7.558e-16, corr = 0.07983669",
     vertex.label = V(graph)$Gene,
     vertex.color = ifelse(V(graph)$Gene == "SGK1", "red", "blue"),
     vertex.size = 28,
     edge.label = E(graph)$Correlation,
